#!/usr/bin/env python3
"""
train_gpt_mlx_no_last_attn.py — Frame + Attention Gate + Surgical L11 Removal

This is a copy of train_gpt_mlx_attn_gate.py with ONE additional change motivated
by the gate analysis: the last block's attention is REMOVED entirely, and its
freed parameter budget is moved into a wider MLP.

WHY: Our previous experiment (train_gpt_mlx_attn_gate.py) added a per-head
content-dependent gate to attention output. After training, we analyzed the
learned gates and found:

    L11 (last layer) gate values:
      H0=0.000  H1=0.000  H2=0.000  H3=0.000
      H4=0.000  H5=0.000  H6=0.000  H7=0.000
      → mean=0.000, std=0.004

The model learned that the FINAL layer's attention is COMPLETELY USELESS and
suppressed it to zero across every head, every token, every input. This is
consistent with the hypothesis that late layers do "translation" (representation
→ vocabulary) rather than "context aggregation".

Since we now KNOW L11 attention is unused, we can surgically remove it and
redirect those parameters to L11's MLP, which IS still used for translation.

ARCHITECTURAL CHANGE:
  Frame+Gate L11:  attention(786K) + gate(4K) + MLP at 2x(1,049K) = 1,839K params
  This script L11: attention REMOVED + MLP at 4x(2,097K)          = 2,097K params
  Net change: +258K params on L11 (slight increase)
  Total model: ~20.78M (vs Frame+Gate at 20.52M, ~1.3% larger)

We don't get exact parameter parity because integer mlp_mult constraints, but
the comparison is close enough to attribute any improvement to the architectural
change rather than to "more parameters". (And if anything, more params on L11's
MLP is exactly what we want — to compensate for losing attention.)

DEFAULT CONFIG:
  - 12 layers
  - HOURGLASS_MLP_MULTS="2,2,2,2,1,1,1,1,2,2,2,4"  ← L11 is now 4x (not 2x)
  - REMOVE_LAST_ATTN=1                              ← skip attention in L11
  - Otherwise identical to Frame+Gate
...
class Hyperparameters:
    ...
    # Frame mults with L11 widened to 4x to absorb the freed attention budget.
    hourglass_mlp_mults: str = os.environ.get("HOURGLASS_MLP_MULTS", "2,2,2,2,1,1,1,1,2,2,2,4")
    disable_skip_connections: bool = bool(int(os.environ.get("DISABLE_SKIP_CONNECTIONS", "1")))
    # Surgically remove the last block's attention (motivated by attn_gate analysis
    # showing L11 gate=0 across all heads). The freed parameters go into a wider L11 MLP.
    remove_last_attn: bool = bool(int(os.environ.get("REMOVE_LAST_ATTN", "1")))
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
        ...
        self.remove_last_attn = remove_last_attn
        ...
        # Last block has its attention removed if remove_last_attn=True
        self.blocks = [
            Block(
                dim, num_heads, num_kv_heads, mlp_mults_per_layer[i], rope_base, qk_gain_init,
                disable_attention=(remove_last_attn and i == num_layers - 1),
            )
            for i in range(num_layers)
        ]
...
def main() -> None:
    ...
    model = GPT(
        ...
        remove_last_attn=args.remove_last_attn,
    )
    log(f"stp_beta:{args.stp_beta} stp_layer:{args.stp_layer} hourglass_mlp_mults:{mlp_mults} disable_skips:{args.disable_skip_connections} remove_last_attn:{args.remove_last_attn}")
