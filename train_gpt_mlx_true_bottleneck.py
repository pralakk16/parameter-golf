...
class Hyperparameters:
    ...
    # True bottleneck — compress model dimension in middle layers
    bottleneck_dim: int = int(os.environ.get("BOTTLENECK_DIM", 256))  # model dim for narrow layers
    wide_layers: int = int(os.environ.get("WIDE_LAYERS", 4))  # layers per wide stage
    narrow_layers: int = int(os.environ.get("NARROW_LAYERS", 4))  # layers in narrow stage
    narrow_mlp_mult: int = int(os.environ.get("NARROW_MLP_MULT", 2))  # MLP expansion in narrow layers
    narrow_num_heads: int = int(os.environ.get("NARROW_NUM_HEADS", 4))  # attention heads in narrow layers
    narrow_num_kv_heads: int = int(os.environ.get("NARROW_NUM_KV_HEADS", 2))  # KV heads in narrow layers
...
class GPT(nn.Module):
    # True Bottleneck: compress model dimension in middle layers.
    # Wide stages (512 dims) → compress → narrow stage (256 dims) → expand → wide stages (512 dims)
    # No bypass possible — even the residual stream is compressed.
    def __init__(self, vocab_size: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 logit_chunk_tokens: int, logit_softcap: float, rope_base: float, tied_embed_init_std: float,
                 qk_gain_init: float, stp_beta: float = 0.005, stp_layer: str = "last",
                 bottleneck_dim: int = 256, wide_layers: int = 4, narrow_layers: int = 4,
                 narrow_mlp_mult: int = 2, narrow_num_heads: int = 4, narrow_num_kv_heads: int = 2):
        super().__init__()
        ...
        self.wide_layers = wide_layers
        self.narrow_layers = narrow_layers
        self.dim = dim
        self.bottleneck_dim = bottleneck_dim

        self.tok_emb = nn.Embedding(vocab_size, dim)

        # Wide blocks (recognition stage) — full model dimension
        self.blocks_wide1 = [
            Block(dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init)
            for _ in range(wide_layers)
        ]

        # Compression: dim → bottleneck_dim
        self.compress = CastedLinear(dim, bottleneck_dim)

        # Narrow blocks (abstraction stage) — compressed dimension
        self.blocks_narrow = [
            Block(bottleneck_dim, narrow_num_heads, narrow_num_kv_heads, narrow_mlp_mult, rope_base, qk_gain_init)
            for _ in range(narrow_layers)
        ]

        # Expansion: bottleneck_dim → dim
        self.expand = CastedLinear(bottleneck_dim, dim)

        # Wide blocks (prediction stage) — full model dimension
        self.blocks_wide2 = [
            Block(dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init)
            for _ in range(wide_layers)
        ]
        ...
        for blocks in [self.blocks_wide1, self.blocks_narrow, self.blocks_wide2]:
            for b in blocks:
    ...
    def _forward(self, input_ids: mx.array, collect_hidden: bool = False):
        x = rms_norm(self.tok_emb(input_ids).astype(COMPUTE_DTYPE))
        x0 = x
        hidden_states = [] if collect_hidden else None

        # Stage 1: Wide (recognition) — 512 dims
        for block in self.blocks_wide1:
            x = block(x, x0)
            if collect_hidden:
        ...
        # Compress: 512 → 256
        x = self.compress(x)
        x0_narrow = x  # new reference for narrow blocks

        # Stage 2: Narrow (abstraction) — 256 dims
        for block in self.blocks_narrow:
            x = block(x, x0_narrow)
            if collect_hidden:
                hidden_states.append(x)
        ...
        # Expand: 256 → 512
        x = self.expand(x)
        x0_wide2 = x  # new reference for final wide blocks

        # Stage 3: Wide (prediction) — 512 dims
        for block in self.blocks_wide2:
            x = block(x, x0_wide2)
            if collect_hidden:
                hidden_states.append(x)
    ...
    def __call__(self, input_ids: mx.array) -> mx.array:
        return self._forward(input_ids, collect_hidden=False)

    def eval_loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        """Pure CE for evaluation."""
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        """Training loss: CE + STP."""
        if self.stp_beta > 0:
            final_output, hidden_states = self._forward(input_ids, collect_hidden=True)
            x = final_output.reshape(-1, self.dim)
        else:
            x = self(input_ids).reshape(-1, self.dim)
        ...
        if self.stp_beta > 0:
            ...
            elif self.stp_layer == "narrow":
                # STP on narrow layers specifically
                narrow_start = self.wide_layers
                narrow_end = self.wide_layers + self.narrow_layers
                stp_loss = mx.mean(mx.array([
                    stp_smoothness_loss(hidden_states[i].astype(mx.float32))
                    for i in range(narrow_start, narrow_end)
                ]))
            else:
                stp_loss = stp_smoothness_loss(hidden_states[int(self.stp_layer)].astype(mx.float32))
            ce_loss = ce_loss + self.stp_beta * stp_loss
...
class SplitOptimizers:
    ...
    def __init__(self, model: GPT, args: Hyperparameters):
        ...
        block_prefixes = ("blocks.", "blocks_wide1.", "blocks_narrow.", "blocks_wide2.", "compress.", "expand.")
        self.matrix_keys = [
            k
            for k, p in params.items()
            if any(k.startswith(pfx) for pfx in block_prefixes) and p.ndim == 2 and not any(pattern in k for pattern in CONTROL_TENSOR_NAME_PATTERNS)
        ]
        self.scalar_keys = [
            k
            for k, p in params.items()
            if k == "skip_weights" or (any(k.startswith(pfx) for pfx in block_prefixes) and (p.ndim < 2 or any(pattern in k for pattern in CONTROL_TENSOR_NAME_PATTERNS)))
        ]
...
def main() -> None:
    ...
    model = GPT(
        ...
        wide_layers=args.wide_layers,
        narrow_layers=args.narrow_layers,
        narrow_mlp_mult=args.narrow_mlp_mult,
        narrow_num_heads=args.narrow_num_heads,
        narrow_num_kv_heads=args.narrow_num_kv_heads,
    )
    total_layers = args.wide_layers * 2 + args.narrow_layers
    log(f"true_bottleneck: {args.wide_layers}W({args.model_dim}d) + {args.narrow_layers}N({args.bottleneck_dim}d) + {args.wide_layers}W({args.model_dim}d) = {total_layers} layers")
    log(f"stp_beta:{args.stp_beta} stp_layer:{args.stp_layer}")
    ...
    log(
        ...
        f"linear_weight:{model.blocks_wide1[0].attn.c_q.weight.dtype} "
        f"compress_weight:{model.compress.weight.dtype}"
    )
