...
class Hyperparameters:
    ...
    # STP — smoothness constraint on hidden states
    stp_beta: float = float(os.environ.get("STP_BETA", 0.005))

    # EMA self-distillation — evolving soft targets from smoothed model copy
    ema_distill_alpha: float = float(os.environ.get("EMA_DISTILL_ALPHA", 0.1))  # weight of distillation loss
    ema_decay: float = float(os.environ.get("EMA_DECAY", 0.999))  # EMA smoothing factor
    ema_distill_temp: float = float(os.environ.get("EMA_DISTILL_TEMP", 3.0))  # temperature for soft targets
...
def stp_smoothness_loss(hidden_states: mx.array) -> mx.array:
    """STP smoothness constraint on hidden state trajectories."""
...
def kl_divergence_loss(student_logits: mx.array, teacher_logits: mx.array, temperature: float) -> mx.array:
    """KL divergence between teacher and student distributions at given temperature."""
    s_logits = student_logits.astype(mx.float32) / temperature
    t_logits = teacher_logits.astype(mx.float32) / temperature
    # Stable log-softmax for both
    s_log_probs = s_logits - mx.logsumexp(s_logits, axis=-1, keepdims=True)
    t_log_probs = t_logits - mx.logsumexp(t_logits, axis=-1, keepdims=True)
    t_probs = mx.exp(t_log_probs)
    # KL(teacher || student) = sum(teacher * (log_teacher - log_student))
    kl = mx.sum(t_probs * (t_log_probs - s_log_probs), axis=-1)
    return mx.mean(kl) * (temperature * temperature)


class GPT(nn.Module):
    # STP + EMA self-distillation: smoothness constraint + evolving soft targets
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 logit_chunk_tokens: int, logit_softcap: float, rope_base: float, tied_embed_init_std: float,
                 qk_gain_init: float, stp_beta: float = 0.005, ema_distill_alpha: float = 0.1,
                 ema_distill_temp: float = 3.0):
        super().__init__()
        ...
        self.ema_distill_alpha = ema_distill_alpha
        self.ema_distill_temp = ema_distill_temp
        ...
        # EMA teacher logits — stored externally, not a model parameter
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        """Training loss: CE + STP smoothness + EMA distillation."""
        # Forward with hidden states for STP
        if self.stp_beta > 0:
        ...
        # EMA distillation loss (only if we have teacher logits from previous step)
        if self.ema_distill_alpha > 0 and self._ema_logits is not None:
            distill_loss = kl_divergence_loss(logits, self._ema_logits, self.ema_distill_temp)
            ce_loss = ce_loss + self.ema_distill_alpha * distill_loss

        # Store current logits for EMA update (detached, used next step)
...
def main() -> None:
    ...
    model = GPT(
        ...
        ema_distill_alpha=args.ema_distill_alpha,
        ema_distill_temp=args.ema_distill_temp,
    )
    log(f"stp_beta:{args.stp_beta} ema_distill_alpha:{args.ema_distill_alpha} ema_decay:{args.ema_decay} ema_distill_temp:{args.ema_distill_temp}")
    ...
    # Pure CE for evaluation, hybrid for training
    ...
    while True:
        ...
        # Update EMA teacher logits: smoothed version of student's current logits
        if args.ema_distill_alpha > 0 and hasattr(model, '_current_logits') and model._current_logits is not None:
            if model._ema_logits is None:
                model._ema_logits = mx.stop_gradient(model._current_logits)
            else:
                model._ema_logits = mx.stop_gradient(
                    args.ema_decay * model._ema_logits + (1.0 - args.ema_decay) * model._current_logits
                )
            mx.eval(model._ema_logits)
