...
class CausalSelfAttention(nn.Module):
    """DeltaNet layer: delta-rule linear recurrence replacing softmax attention.

    Instead of computing pairwise attention scores (O(n²)), maintains a
    fixed-size memory matrix S per head that gets updated at each token
    via error-correcting delta rule:

        old_value = S @ k_t           (what's currently stored at this key)
        delta     = v_t - old_value   (how wrong the memory is)
        S         = S + beta * outer(k_t, delta)   (correct the memory)
        output    = q_t @ S           (read from updated memory)

    This is O(n) per layer (constant work per token) and inherently causal
    (each token only sees the memory built from previous tokens).

    Keeps the per-head confidence gate from Frame+Gate.
    Uses GQA: num_kv_heads memories, num_heads queries grouped in pairs.
    """
    def __init__(
        self,
        dim: int,
        ...
        # Q/K/V projections — same sizes as the attention version
        self.c_q = CastedLinear(dim, dim)
        self.c_k = CastedLinear(dim, kv_dim)
        self.c_v = CastedLinear(dim, kv_dim)
        self.proj = CastedLinear(dim, dim)
        # DeltaNet-specific: per-KV-head update gate (beta)
        self.c_beta = CastedLinear(dim, num_kv_heads)
        # Per-head confidence gate (our contribution from Frame+Gate)
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        # Project Q, K, V
        q = rms_norm(self.c_q(x).reshape(bsz, seqlen, self.num_heads, d)).astype(COMPUTE_DTYPE)
        k_raw = self.c_k(x).reshape(bsz, seqlen, nkv, d).astype(COMPUTE_DTYPE)
        # L2-normalize keys (critical for numerical stability — bounds memory updates)
        k = k_raw / (mx.sqrt(mx.sum(k_raw * k_raw, axis=-1, keepdims=True)) + 1e-6)
        v = self.c_v(x).reshape(bsz, seqlen, nkv, d).astype(COMPUTE_DTYPE)
        beta = mx.sigmoid(self.c_beta(x).astype(mx.float32)).astype(COMPUTE_DTYPE)
        # beta shape: (bsz, seqlen, nkv)

        # Initialize memory: (bsz, nkv, d, d) — one d×d matrix per KV head
        S = mx.zeros((bsz, nkv, d, d), dtype=COMPUTE_DTYPE)

        # Sequential delta-rule processing (naive implementation for MLX)
        outputs = []
        for t in range(seqlen):
            q_t = q[:, t, :, :]    # (bsz, num_heads, d)
            k_t = k[:, t, :, :]    # (bsz, nkv, d)
            v_t = v[:, t, :, :]    # (bsz, nkv, d)
            beta_t = beta[:, t, :] # (bsz, nkv)

            # Delta rule update FIRST (so token t's value is written before reading)
            # old_value = k_t @ S: what's currently stored at this key
            old_val = (k_t[:, :, None, :] @ S).squeeze(2)  # (bsz, nkv, d)
            delta = v_t - old_val  # prediction error

            # S += beta * outer(k, delta): error-correcting update
            update = k_t[:, :, :, None] * delta[:, :, None, :]  # (bsz, nkv, d, d)
            S = S + beta_t[:, :, None, None] * update

            # Read from UPDATED memory for each query head (grouped by KV head)
            # Expand S for grouped queries: (bsz, nkv, d, d) → (bsz, num_heads, d, d)
            S_exp = mx.repeat(S, hpg, axis=1)
            # out_t = q_t @ S: (bsz, num_heads, 1, d) @ (bsz, num_heads, d, d) → (bsz, num_heads, d)
            out_t = (q_t[:, :, None, :] @ S_exp).squeeze(2)  # (bsz, num_heads, d)

            outputs.append(out_t)

        # Stack: (bsz, seqlen, num_heads, d)
        y = mx.stack(outputs, axis=1)

        # Scale output
        y = y * self.scale

        # Per-head confidence gate (our contribution)
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
        ...
        self.blocks = [
            Block(dim, num_heads, num_kv_heads, mlp_mults_per_layer[i], rope_base, qk_gain_init)
            for i in range(num_layers)
        ]
