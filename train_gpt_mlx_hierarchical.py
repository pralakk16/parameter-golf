#!/usr/bin/env python3
"""
train_gpt_mlx_attn_gate.py — Frame + Attention Confidence Gate

This is a copy of train_gpt_mlx_hourglass_stp.py (Frame + STP + no skips, our
current best at val_bpb 1.7473) with ONE architectural change:

  CausalSelfAttention now has a content-dependent per-head GATE projection that
  multiplies the attention output before the output projection.
...
class Block(nn.Module):
    """Standard transformer block with attention + MLP + per-head gate."""
    def __init__(
        self,
        dim: int,
        num_heads: int,
...
class MLPBlock(nn.Module):
    """MLP-only block (no attention). Used at coarse levels in the hierarchical model.
    Wider MLP to compensate for the lack of attention capacity."""
    def __init__(self, dim: int, mlp_mult: int):
        super().__init__()
...
class GPT(nn.Module):
    """Hierarchical sequence-length model with multi-scale processing.

    Architecture:
      Level 0 (1024 tokens): 2 attention blocks with per-head gate (fine-grained)
      Pool: take odd positions → 512 (causal: each odd position has seen its pair)
      Level 1 (512 tokens):  2 MLP-only blocks with wider MLP (phrase-level)
      Pool: take odd positions → 256
      Level 2 (256 tokens):  2 MLP-only blocks with wider MLP (sentence-level)
      Broadcast: causally shift coarse info back to 1024 positions
      Decode (1024 tokens):  2 attention blocks (final output generation)

    Total: 4 attention blocks + 4 MLP-only blocks = 8 blocks
    Attention is only at the finest level (2 encode + 2 decode), saving ~60% of
    attention cost vs a 12-layer transformer. Coarse levels use cheap MLPs.
    """
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
        ...
        # Coarse MLP mult — wider to compensate for no attention and match ~20M param budget.
        coarse_mlp_mult = int(os.environ.get("COARSE_MLP_MULT", "6"))
        fine_mlp_mult = mlp_mult  # typically 2

        # All blocks in one flat list for optimizer compatibility (keys start with "blocks.")
        # Layout: [enc0_attn, enc0_attn, enc1_mlp, enc1_mlp, enc2_mlp, enc2_mlp, dec0_attn, dec0_attn]
        self.blocks = [
            # Encode Level 0: 2 attention blocks on full 1024 tokens
            Block(dim, num_heads, num_kv_heads, fine_mlp_mult, rope_base, qk_gain_init),
            Block(dim, num_heads, num_kv_heads, fine_mlp_mult, rope_base, qk_gain_init),
            # Encode Level 1: 2 MLP-only blocks on 512 tokens (phrase-level)
            MLPBlock(dim, coarse_mlp_mult),
            MLPBlock(dim, coarse_mlp_mult),
            # Encode Level 2: 2 MLP-only blocks on 256 tokens (sentence-level)
            MLPBlock(dim, coarse_mlp_mult),
            MLPBlock(dim, coarse_mlp_mult),
            # Decode Level 0: 2 attention blocks on full 1024 tokens (output generation)
            Block(dim, num_heads, num_kv_heads, fine_mlp_mult, rope_base, qk_gain_init),
            Block(dim, num_heads, num_kv_heads, fine_mlp_mult, rope_base, qk_gain_init),
        ]

        # Learned scales for combining multi-scale info (init 0 = start with fine only)
        self.coarse1_scale = mx.zeros((dim,), dtype=mx.float32)
        self.coarse2_scale = mx.zeros((dim,), dtype=mx.float32)
    ...
    @staticmethod
    def _causal_pool(x: mx.array) -> mx.array:
        """Pool by taking odd positions. After causal attention, each odd position
        x[2j+1] contains a causal summary of tokens [0..2j+1].
        Output has half the sequence length."""
        return x[:, 1::2, :]

    @staticmethod
    def _causal_broadcast(coarse: mx.array, target_len: int, stride: int) -> mx.array:
        """Broadcast coarse representations back to fine resolution, causally.

        coarse[j] summarizes tokens [0..j*stride + stride - 1].
        For output position t, we use coarse[j] where j*stride + stride - 1 <= t,
        i.e., the latest chunk that has fully completed by position t.
        This is achieved by repeating each coarse element 'stride' times and
        shifting right by (stride-1) positions.
        """
        batch, coarse_len, dim = coarse.shape
        # Repeat each coarse element 'stride' times
        repeated = mx.repeat(coarse, stride, axis=1)  # (batch, coarse_len*stride, dim)
        # Shift right by (stride-1) for causality
        pad = mx.zeros((batch, stride - 1, dim), dtype=repeated.dtype)
        shifted = mx.concatenate([pad, repeated], axis=1)[:, :target_len, :]
        return shifted
    ...
    def _hierarchical_forward(self, input_ids: mx.array) -> tuple[mx.array, mx.array]:
        """Hierarchical forward pass. Returns (final_output, last_hidden_for_stp)."""
        x = rms_norm(self.tok_emb(input_ids).astype(COMPUTE_DTYPE))
        x0 = x
        seq_len = x.shape[1]

        # === ENCODE Level 0: attention on full 1024 tokens ===
        x = self.blocks[0](x, x0)
        x = self.blocks[1](x, x0)
        h0 = x  # save fine-grained representation

        # === Pool to Level 1: 1024 → 512 ===
        x1 = self._causal_pool(x)  # (batch, 512, dim)

        # === ENCODE Level 1: MLP on 512 chunks ===
        x1 = self.blocks[2](x1)
        x1 = self.blocks[3](x1)
        h1 = x1

        # === Pool to Level 2: 512 → 256 ===
        x2 = self._causal_pool(x1)  # (batch, 256, dim)

        # === ENCODE Level 2: MLP on 256 chunks ===
        x2 = self.blocks[4](x2)
        x2 = self.blocks[5](x2)
        h2 = x2

        # === BROADCAST coarse info back to full resolution (causal) ===
        h1_broadcast = self._causal_broadcast(h1, seq_len, stride=2)
        h2_broadcast = self._causal_broadcast(h2, seq_len, stride=4)

        # === COMBINE: fine + multi-scale coarse info ===
        x = h0 + self.coarse1_scale.astype(h0.dtype)[None, None, :] * h1_broadcast \
              + self.coarse2_scale.astype(h0.dtype)[None, None, :] * h2_broadcast

        # === DECODE Level 0: attention on full 1024 tokens ===
        x = self.blocks[6](x, x0)
        x = self.blocks[7](x, x0)

        return self.final_norm(x), x

    def eval_loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        """Pure cross-entropy loss for evaluation — no STP term."""
        final_output, _ = self._hierarchical_forward(input_ids)
    ...
    def forward_with_hidden(self, input_ids: mx.array) -> tuple[mx.array, list[mx.array]]:
        """Forward pass that also returns hidden states (for STP)."""
        final_output, last_hidden = self._hierarchical_forward(input_ids)
        return final_output, [last_hidden]
...
def main() -> None:
    ...
    log(
        ...
        f"coarse_scales:{model.coarse1_scale.dtype}"
    )
