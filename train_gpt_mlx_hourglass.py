...
class Hyperparameters:
    ...
    # Hourglass MLP — vary MLP width per layer to create hierarchical stages
    # Format: comma-separated multipliers per layer, or "uniform" for standard
    # Example: "2,2,2,1,1,1,2,2,2" for hourglass with 9 layers
...
def parse_hourglass_mlp_mults(config: str, num_layers: int, default_mult: int) -> list[int]:
    """Parse hourglass MLP multiplier configuration.

    'uniform' → all layers use default_mult
    '2,2,2,1,1,1,2,2,2' → per-layer multipliers
    'hourglass' → automatic hourglass: wide-narrow-wide pattern
    """
    if config == "uniform":
        return [default_mult] * num_layers
    if config == "hourglass":
        # Automatic hourglass: first third wide, middle third narrow, last third wide
        third = num_layers // 3
        remainder = num_layers - 3 * third
        mults = []
        for i in range(num_layers):
            if i < third + (1 if remainder > 0 else 0):
                mults.append(default_mult)  # wide (recognition)
            elif i < 2 * third + (1 if remainder > 0 else 0):
                mults.append(max(1, default_mult // 2))  # narrow (abstraction)
            else:
                mults.append(default_mult)  # wide (prediction)
        return mults
    # Explicit comma-separated values
    mults = [int(x.strip()) for x in config.split(",")]
    if len(mults) != num_layers:
        raise ValueError(f"HOURGLASS_MLP_MULTS has {len(mults)} values but NUM_LAYERS={num_layers}")
    return mults


class GPT(nn.Module):
    # Hourglass MLP: different MLP widths per layer to create
    # recognition → abstraction → prediction hierarchy.
    # Narrow middle layers force the model to form abstract representations.
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 logit_chunk_tokens: int, logit_softcap: float, rope_base: float, tied_embed_init_std: float,
                 qk_gain_init: float, mlp_mults_per_layer: list[int] | None = None):
        super().__init__()
        ...
        # Use per-layer MLP mults if provided, otherwise uniform
        if mlp_mults_per_layer is None:
            mlp_mults_per_layer = [mlp_mult] * num_layers
        self.mlp_mults_per_layer = mlp_mults_per_layer
...
def main() -> None:
    ...
    log(f"hourglass_mlp_mults:{mlp_mults}")
