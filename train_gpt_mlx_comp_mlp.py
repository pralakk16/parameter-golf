#!/usr/bin/env python3
"""
train_gpt_mlx_comp_mlp.py — Frame + Attention Gate + Compositional MLP

Built on top of Frame+Gate (val_bpb 1.7396, our current best). Adds ONE more
architectural change: Compositional MLP.

WHY: Standard transformer MLPs at each layer learn independent dictionaries of
patterns. Layer 5's W_in has 1024 patterns; Layer 6's W_in has 1024 different
patterns; the two layers cannot reuse each other's patterns. This causes
diminishing returns with depth: each new layer rediscovers things earlier
layers already know.

INSIGHT: Knowledge should COMPOUND, not just accumulate. Each layer's patterns
should be COMPOSED FROM previous layers' patterns (plus its own new ones). This
is how human learning works (digits to numbers to equations to theorems).

ARCHITECTURE:
For each MLP at layer L, we compute:
    own_expand_L = relu(W_in_L @ x)              # standard pattern matches
    composed_L   = sum_j w_{L,j} * expand_j      # weighted sum of previous
                                                   same-shape layers' patterns
    expand_L     = own_expand_L + alpha_L * composed_L
    output_L     = W_out_L @ expand_L^2

Composition happens at the post-ReLU level (where "matched patterns" live as
sparse positive activations). Each layer L has:
  - w_{L,j}: learnable scalar per previous same-shape layer j (init small random)
  - alpha_L: learnable scalar gating composition strength (init 0.1)

CONSTRAINTS:
  - Compose only WITHIN same hidden-size group (wide-with-wide, narrow-with-narrow)
    to avoid dimension mismatches
  - Wide layers in our 12L hourglass: L0,L1,L2,L3,L8,L9,L10,L11 (hidden=1024)
  - Narrow layers: L4,L5,L6,L7 (hidden=512)
  - L4 composes from {} (first narrow), L5 from {L4}, L6 from {L4,L5}, etc.
  - L1 composes from {L0}, L2 from {L0,L1}, L3 from {L0,L1,L2},
    L8 composes from {L0,L1,L2,L3}, L9 from {L0..L3,L8}, etc.

PARAM COST: ~28-50 extra scalars total across the entire model. Effectively free.

DEFAULT CONFIG: identical to Frame+Gate.
  - 12L hourglass MLP "2,2,2,2,1,1,1,1,2,2,2,2"
  - U-Net skips disabled
  - STP beta=0.005 on last layer
  - Per-head attention gate (from attn_gate)
  - NEW: compositional MLP within same-shape groups

Motivation: standard QKV attention always outputs *something* for every token.
The model has to learn to suppress unhelpful attention via the V projection
and the output projection. We give it a more direct mechanism: a per-token,
per-head sigmoid gate that says "how much do I trust this head's attention
output for THIS token in THIS context?" The gate is computed from the input
token's hidden state via a tiny new projection (dim → num_heads), so it's
content-dependent and almost free in parameters.

Mathematical change: where standard attention does
    y = SDPA(Q, K, V)            # (bsz, num_heads, seqlen, head_dim)
    out = proj(reshape(y))
We now do
    y = SDPA(Q, K, V)
    g = sigmoid(c_g(x))           # (bsz, seqlen, num_heads)  ← NEW
    y = y * g[:, :, :, None]      # per-head gating, broadcast over head_dim
    out = proj(reshape(y))

Inductive bias: the model can learn that for some tokens the attention is
unreliable (no clear match) and should be suppressed, and for others it's
critical and should be amplified. This is the GLU/gating pattern from MLPs
applied to attention output. The gate is a 1-D scalar per head (not per
head_dim element) to keep it cheap and to push the model to make a clean
trust/distrust decision per head.

Parameter cost: dim × num_heads = 512 × 8 = 4,096 params per block,
× 12 blocks = ~49K params. About 0.24% of the model. Almost free.

Default config inherits Frame: 12L, hourglass MLP "2,2,2,2,1,1,1,1,2,2,2,2",
STP β=0.005 on last layer, U-Net skips DISABLED, TRAIN_BATCH_TOKENS=8192
(matching Frame's actual previous run config).

Run command:
    PYTHONUNBUFFERED=1 RUN_ID=attn_gate_v1 ITERATIONS=1000 NUM_LAYERS=12 \\
      ...
      TRAIN_LOG_EVERY=50 \\
      .venv/bin/python -u train_gpt_mlx_attn_gate.py > logs/attn_gate_v1.log 2>&1
"""
...
class Hyperparameters:
    ...
    # Training loop. Defaults match Frame's previous successful run config:
    # - 1000 iterations cap, 8192 train tokens/step, 65536 val batch tokens
    # - This mirrors the run that produced val_bpb 1.7473
    iterations: int = int(os.environ.get("ITERATIONS", 1_000))
    ...
    # Model (defaults match Frame: 12L hourglass with skips disabled and STP).
    ...
    # Semantic Tube Prediction (STP) — JEPA-inspired auxiliary loss
    stp_beta: float = float(os.environ.get("STP_BETA", 0.005))
    stp_layer: str = os.environ.get("STP_LAYER", "last")

    # Hourglass MLP — Frame default (wide-narrow-wide), 12L for parity with Frame run.
    hourglass_mlp_mults: str = os.environ.get("HOURGLASS_MLP_MULTS", "2,2,2,2,1,1,1,1,2,2,2,2")
    disable_skip_connections: bool = bool(int(os.environ.get("DISABLE_SKIP_CONNECTIONS", "1")))
    stp_layer: str = os.environ.get("STP_LAYER", "last")  # "last", "all", or layer index
...
class CausalSelfAttention(nn.Module):
    ...
    # - RoPE on q and k
    # - causal masked SDPA
    # - NEW: per-head content-dependent confidence gate on attention output
    def __init__(
        self,
        dim: int,
        num_heads: int,
        ...
        # NEW: per-head content-dependent gate.
        # c_g maps the input token (dim) to one scalar per head (num_heads),
        # which after sigmoid acts as a per-token confidence gate on each
        # head's attention output. Cheap (~4K params per block) and lets the
        # model directly suppress unreliable attention heads on a per-token basis.
        self.c_g = CastedLinear(dim, num_heads)
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        # Per-head, per-token content-dependent gate.
...
class CompositionalMLP(nn.Module):
    """MLP with optional composition of previous same-shape layers' patterns.

    The standard MLP computes:  output = W_out @ relu(W_in @ x)^2

    This MLP additionally takes a list of `prev_expands` (post-ReLU expand
    outputs of previous same-shape MLPs in the network) and forms a learnable
    weighted combination of them, added to its own expand BEFORE the squaring
    and final projection. This lets the layer build new "patterns" out of
    previous layers' patterns instead of learning everything from scratch.
    """

    def __init__(self, dim: int, mlp_mult: int, num_prev_same_shape: int):
        super().__init__()
        self.hidden = dim * mlp_mult
        self.fc = CastedLinear(dim, self.hidden)
        self.proj = CastedLinear(self.hidden, dim)
        self.num_prev_same_shape = num_prev_same_shape
        if num_prev_same_shape > 0:
            # Per-prev-layer composition weights, init very small (~0.01)
            init = np.random.RandomState(42 + num_prev_same_shape).randn(num_prev_same_shape).astype(np.float32) * 0.01
            self.compose_weights = mx.array(init)
            # Learned scalar that gates the entire composed contribution.
            # Init 0.1 to give the gradient signal a small but non-zero starting point.
            self.compose_alpha = mx.array(np.array([0.1], dtype=np.float32))

    def __call__(self, x: mx.array, prev_expands: list[mx.array] | None = None) -> tuple[mx.array, mx.array]:
        # x: (B, T, dim)
        own_expand = nn.relu(self.fc(x))  # (B, T, hidden)

        if self.num_prev_same_shape > 0 and prev_expands is not None and len(prev_expands) > 0:
            assert len(prev_expands) == self.num_prev_same_shape, (
                f"expected {self.num_prev_same_shape} prev expands, got {len(prev_expands)}"
            )
            # Stack: (num_prev, B, T, hidden)
            stacked = mx.stack(prev_expands, axis=0)
            # Apply per-layer scalar weights: (num_prev,) -> (num_prev, 1, 1, 1)
            weights = self.compose_weights.astype(stacked.dtype)[:, None, None, None]
            composed = (stacked * weights).sum(axis=0)  # (B, T, hidden)

            alpha = self.compose_alpha.astype(own_expand.dtype)
            expand = own_expand + alpha * composed
        else:
            expand = own_expand

        out = self.proj(expand * expand)
        return out, own_expand  # return own_expand so future layers can compose with it
...
class Block(nn.Module):
    def __init__(
        ...
        rope_base: float,
        qk_gain_init: float,
        num_prev_same_shape: int = 0,
    ):
        super().__init__()
        self.attn_norm = RMSNormNoWeight()
        self.mlp_norm = RMSNormNoWeight()
        self.attn = CausalSelfAttention(dim, num_heads, num_kv_heads, rope_base, qk_gain_init)
        self.mlp = CompositionalMLP(dim, mlp_mult, num_prev_same_shape)
        self.attn_scale = mx.ones((dim,), dtype=mx.float32)
        self.mlp_scale = mx.ones((dim,), dtype=mx.float32)
        self.resid_mix = mx.array(np.stack((np.ones((dim,), dtype=np.float32), np.zeros((dim,), dtype=np.float32))))

    def __call__(self, x: mx.array, x0: mx.array, prev_expands: list[mx.array] | None = None) -> tuple[mx.array, mx.array]:
        mix = self.resid_mix.astype(x.dtype)
        x = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
        attn_out = self.attn(self.attn_norm(x))
        x = x + self.attn_scale.astype(x.dtype)[None, None, :] * attn_out
        mlp_out, own_expand = self.mlp(self.mlp_norm(x), prev_expands=prev_expands)
        x = x + self.mlp_scale.astype(x.dtype)[None, None, :] * mlp_out
        return x, own_expand


def stp_smoothness_loss(hidden_states: mx.array) -> mx.array:
    """Semantic Tube Prediction — JEPA-inspired smoothness constraint.

    Constrains hidden state trajectories to be locally linear (geodesic).
    Based on: if language meaning changes smoothly, hidden states should too.
    The second-order finite difference measures deviation from linearity:
        acceleration = h[t+1] - 2*h[t] + h[t-1]
    Minimizing this forces the trajectory to be a smooth curve.

    Args:
        hidden_states: (batch, seq_len, dim) hidden states from a transformer layer

    Returns:
        scalar smoothness loss (mean squared acceleration)
    """
    # h[t-1], h[t], h[t+1] for all valid positions
    h_prev = hidden_states[:, :-2, :]   # positions 0 to seq_len-3
    h_curr = hidden_states[:, 1:-1, :]  # positions 1 to seq_len-2
    h_next = hidden_states[:, 2:, :]    # positions 2 to seq_len-1

    # Second-order finite difference (acceleration)
    # If trajectory is perfectly linear: acceleration = 0
    acceleration = h_next - 2.0 * h_curr + h_prev

    # Mean squared acceleration across all positions, batch, and dimensions
    return mx.mean(acceleration * acceleration)


def parse_hourglass_mlp_mults(config: str, num_layers: int, default_mult: int) -> list[int]:
    """Parse hourglass MLP configuration."""
    if config == "uniform":
        return [default_mult] * num_layers
    if config == "hourglass":
        third = num_layers // 3
        mults = []
        for i in range(num_layers):
            if i < third:
                ...
                mults.append(max(1, default_mult // 2))
            else:
                mults.append(default_mult)
        return mults
    return [int(x.strip()) for x in config.split(",")]


class GPT(nn.Module):
    # Hourglass MLP + STP: hierarchical MLP widths + smoothness constraint
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 logit_chunk_tokens: int, logit_softcap: float, rope_base: float, tied_embed_init_std: float,
                 qk_gain_init: float, stp_beta: float = 0.005, stp_layer: str = "last",
                 mlp_mults_per_layer: list[int] | None = None, disable_skip_connections: bool = False):
        super().__init__()
        ...
        self.stp_layer = stp_layer
        self.disable_skip_connections = disable_skip_connections

        if mlp_mults_per_layer is None:
            mlp_mults_per_layer = [mlp_mult] * num_layers
        ...
        # For Compositional MLP: count, for each layer, how many earlier layers
        # have the SAME mlp_mult (i.e., the same expand hidden size). That count
        # is the size of this layer's compose_weights vector.
        self.mlp_mults_per_layer = list(mlp_mults_per_layer)
        prev_same_shape_counts = []
        for i in range(num_layers):
            count = sum(1 for j in range(i) if mlp_mults_per_layer[j] == mlp_mults_per_layer[i])
            prev_same_shape_counts.append(count)
        self.prev_same_shape_counts = prev_same_shape_counts

        self.blocks = [
            Block(
                dim, num_heads, num_kv_heads, mlp_mults_per_layer[i], rope_base, qk_gain_init,
                num_prev_same_shape=prev_same_shape_counts[i],
            )
            for i in range(num_layers)
        ]
    ...
    def eval_loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        """Pure cross-entropy loss for evaluation — no STP term."""
    ...
    def _gather_prev_expands(self, layer_idx: int, expands_by_shape: dict[int, list[mx.array]]) -> list[mx.array]:
        """Return the list of post-ReLU expand outputs from earlier same-shape layers."""
        mult = self.mlp_mults_per_layer[layer_idx]
        return expands_by_shape.get(mult, [])

    def forward_with_hidden(self, input_ids: mx.array) -> tuple[mx.array, list[mx.array]]:
        """Forward pass that also returns hidden states from each block."""
        x = rms_norm(self.tok_emb(input_ids).astype(COMPUTE_DTYPE))
        x0 = x
        skips: list[mx.array] = []
        hidden_states: list[mx.array] = []
        # Per-shape running list of post-ReLU expand outputs for compositional MLP.
        expands_by_shape: dict[int, list[mx.array]] = {}

        for i in range(self.num_encoder_layers):
            prev = self._gather_prev_expands(i, expands_by_shape)
            x, own_expand = self.blocks[i](x, x0, prev_expands=prev)
            mult = self.mlp_mults_per_layer[i]
            expands_by_shape.setdefault(mult, []).append(own_expand)
            skips.append(x)
            hidden_states.append(x)
        for i in range(self.num_decoder_layers):
            if skips and not self.disable_skip_connections:
                x = x + self.skip_weights[i].astype(x.dtype)[None, None, :] * skips.pop()
            elif skips:
                skips.pop()  # consume but don't use
            layer_idx = self.num_encoder_layers + i
            prev = self._gather_prev_expands(layer_idx, expands_by_shape)
            x, own_expand = self.blocks[layer_idx](x, x0, prev_expands=prev)
            mult = self.mlp_mults_per_layer[layer_idx]
            expands_by_shape.setdefault(mult, []).append(own_expand)
            hidden_states.append(x)
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        if self.stp_beta > 0:
            # Use forward_with_hidden to capture hidden states for STP
            final_output, hidden_states = self.forward_with_hidden(input_ids)
            x = final_output.reshape(-1, self.tok_emb.weight.shape[1])
            y = target_ids.reshape(-1)
            ...
            # Compute STP smoothness loss on selected layer(s)
            if self.stp_layer == "last":
                stp_loss = stp_smoothness_loss(hidden_states[-1].astype(mx.float32))
            elif self.stp_layer == "all":
                stp_loss = mx.mean(mx.array([
                    stp_smoothness_loss(h.astype(mx.float32)) for h in hidden_states
                ]))
            else:
                layer_idx = int(self.stp_layer)
                stp_loss = stp_smoothness_loss(hidden_states[layer_idx].astype(mx.float32))

            return ce_loss + self.stp_beta * stp_loss
        else:
            # Pure CE — identical to baseline
            x = self(input_ids).reshape(-1, self.tok_emb.weight.shape[1])
            y = target_ids.reshape(-1)
...
def main() -> None:
    ...
    # ==============================================================================
    mlp_mults = parse_hourglass_mlp_mults(args.hourglass_mlp_mults, args.num_layers, args.mlp_mult)
    model = GPT(
        ...
        stp_beta=args.stp_beta,
        stp_layer=args.stp_layer,
        mlp_mults_per_layer=mlp_mults,
        disable_skip_connections=args.disable_skip_connections,
    )
    log(f"stp_beta:{args.stp_beta} stp_layer:{args.stp_layer} hourglass_mlp_mults:{mlp_mults} disable_skips:{args.disable_skip_connections}")
    ...
    # compiled_loss: pure CE for evaluation (comparable across experiments)
    # compiled_loss_and_grad: CE + STP for training
