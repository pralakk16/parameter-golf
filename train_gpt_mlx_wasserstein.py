...
def compute_cost_matrix(embeddings: mx.array, metric: str = "cosine") -> mx.array:
    ...
    if metric == "cosine":
        ...
        # Cosine distance: 1 - similarity (range: 0 = identical, 2 = opposite)
        cost = 1.0 - similarity
    else:
    ...
    # Ensure diagonal is exactly zero and no negative values
...
def wasserstein_loss(logits: mx.array, targets: mx.array, cost_matrix: mx.array) -> mx.array:
    ...
    This penalizes predictions proportionally to their semantic distance
    from the correct token. Predicting "rug" when the answer is "mat"
    gets a small penalty (close in meaning). Predicting "quantum" gets
    a large penalty (far in meaning).
    ...
    # Look up the cost row for each correct token
    # cost_row[i] = distances from correct_token[i] to all vocab tokens
    cost_rows = cost_matrix[targets]  # (num_tokens, vocab_size)

    # Wasserstein-1 for one-hot targets: sum of P(token) * distance(token, correct)
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
        ...
        # Initialize cost matrix from random embeddings (will be updated during training)
        self._cost_matrix = compute_cost_matrix(self.tok_emb.weight, self.wasserstein_cost_metric)

    def update_cost_matrix(self):
        """Recompute cost matrix from current embeddings.
        Call periodically during training to keep cost matrix in sync."""
        self._cost_matrix = compute_cost_matrix(self.tok_emb.weight, self.wasserstein_cost_metric)
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        # Hybrid cross-entropy + Wasserstein loss.
        # alpha=1.0 is pure cross-entropy (identical to baseline).
        # alpha=0.0 is pure Wasserstein.
        # alpha=0.9 means 90% CE + 10% Wasserstein (default).
        ...
        # If pure cross-entropy, skip Wasserstein computation entirely
        if self.wasserstein_alpha >= 1.0:
            return ce_loss
...
def main() -> None:
    ...
    log(f"wasserstein_alpha:{args.wasserstein_alpha} cost_metric:{args.wasserstein_cost_metric} cost_update_every:{args.wasserstein_cost_update_every}")
