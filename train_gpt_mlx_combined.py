...
class Hyperparameters:
    ...
    # STP (Semantic Tube Prediction) — smoothness constraint on hidden states
    stp_beta: float = float(os.environ.get("STP_BETA", 0.005))
    stp_layer: str = os.environ.get("STP_LAYER", "last")

    # Wasserstein — differentiated weighting of wrong tokens
    wasserstein_alpha: float = float(os.environ.get("WASSERSTEIN_ALPHA", 0.9))
    wasserstein_cost_metric: str = os.environ.get("WASSERSTEIN_COST_METRIC", "cosine")
    wasserstein_cost_update_every: int = int(os.environ.get("WASSERSTEIN_COST_UPDATE_EVERY", 100))
...
def stp_smoothness_loss(hidden_states: mx.array) -> mx.array:
    """Semantic Tube Prediction — smoothness constraint on hidden state trajectories."""
...
def compute_cost_matrix(embeddings: mx.array, metric: str = "cosine") -> mx.array:
    """Compute pairwise distance matrix between token embeddings for Wasserstein loss."""
    ...
    return mx.maximum(cost, 0.0)


def wasserstein_loss(logits: mx.array, targets: mx.array, cost_matrix: mx.array) -> mx.array:
    """Wasserstein-1 loss for one-hot targets: W = sum(P(token) * distance(token, correct))."""
    logits_f32 = logits.astype(mx.float32)
    logits_shifted = logits_f32 - mx.max(logits_f32, axis=-1, keepdims=True)
    exp_logits = mx.exp(logits_shifted)
    probs = exp_logits / mx.sum(exp_logits, axis=-1, keepdims=True)
    cost_rows = cost_matrix[targets]
    per_token_loss = mx.sum(probs * cost_rows, axis=-1)
    return mx.mean(per_token_loss)


class GPT(nn.Module):
    # Combined: STP (smoothness) + Wasserstein (differentiated token weighting)
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 logit_chunk_tokens: int, logit_softcap: float, rope_base: float, tied_embed_init_std: float,
                 qk_gain_init: float, stp_beta: float = 0.005, stp_layer: str = "last",
                 wasserstein_alpha: float = 0.9, wasserstein_cost_metric: str = "cosine"):
        super().__init__()
        ...
        # Cost matrix for Wasserstein (only if not pure CE)
        if wasserstein_alpha < 1.0:
            self._cost_matrix = compute_cost_matrix(self.tok_emb.weight, wasserstein_cost_metric)
        else:
            self._cost_matrix = None
        self._wasserstein_cost_metric = wasserstein_cost_metric

    def update_cost_matrix(self):
        if self._cost_matrix is not None:
            self._cost_matrix = compute_cost_matrix(self.tok_emb.weight, self._wasserstein_cost_metric)
    ...
    def eval_loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        """Pure CE for evaluation — no STP, no Wasserstein."""
    ...
    def loss(self, input_ids: mx.array, target_ids: mx.array) -> mx.array:
        """Training loss: CE + STP smoothness + Wasserstein differentiation."""
        # Get hidden states if STP is enabled
        if self.stp_beta > 0:
        ...
        if self.wasserstein_alpha < 1.0 and self._cost_matrix is not None:
            w_loss = wasserstein_loss(logits, y, self._cost_matrix)
            ce_loss = self.wasserstein_alpha * ce_loss + (1.0 - self.wasserstein_alpha) * w_loss
...
def main() -> None:
    ...
    log(f"stp_beta:{args.stp_beta} stp_layer:{args.stp_layer} wasserstein_alpha:{args.wasserstein_alpha}")
    ...
    # eval uses pure CE; training uses CE + STP + Wasserstein
    ...
    while True:
        ...
        # Periodically recompute Wasserstein cost matrix
