#!/usr/bin/env python3
"""
train_gpt_mlx_crown.py — Crown variant (narrow-wide-narrow MLP shape)

This is a copy of train_gpt_mlx_hourglass_stp.py with one default flipped:
the per-layer MLP multiplier list defaults to a CROWN pattern (narrow at the
ends, wide in the middle) instead of the FRAME pattern (wide at the ends,
narrow in the middle).

Why: Baroian & Notebomer (2025), "Crown, Frame, Reverse: Layer-Wise Scaling
Variants for LLM Pre-Training" (arXiv:2509.06518), tested both shapes at
180M parameters and found Crown was the BEST heterogeneous variant while
Frame was the WORST. The Layerwise Importance Analysis paper (arXiv:2508.17734,
2025) independently confirmed Crown-style middle-heavy FFN allocation wins
at 285M, 570M, and 1.2B scale.

Our existing hourglass result (+0.218 BPB at sub-100M) used the Frame shape.
This script lets us run the head-to-head comparison at the SAME parameter
budget — 8 wide blocks + 4 narrow blocks, identical to Frame, just rearranged.

Default 12L Crown layout (parameter parity with Frame):
    Frame:  2,2,2,2,1,1,1,1,2,2,2,2   (4 wide / 4 narrow / 4 wide)
    Crown:  1,1,2,2,2,2,2,2,2,2,1,1   (2 narrow / 8 wide / 2 narrow)

Both have 8 wide blocks and 4 narrow blocks. Same MLP parameter count.
Same attention parameter count. Same embedding. Only the SHAPE differs.

If Frame still wins at our scale, the scale-reversal hypothesis holds.
If Crown wins (matching the 180M result), our hourglass story breaks and
we should rebuild around Crown instead.

Run command (matches the existing best Frame run):
    RUN_ID=crown ITERATIONS=1000 NUM_LAYERS=12 \\
      HOURGLASS_MLP_MULTS="1,1,2,2,2,2,2,2,2,2,1,1" \\
      STP_BETA=0.005 STP_LAYER=last \\
      DISABLE_SKIP_CONNECTIONS=1 SKIP_ROUNDTRIP=1 \\
      python3 train_gpt_mlx_crown.py
"""
...
class Hyperparameters:
    ...
    # Model (defaults match Frame run for parity: 12 layers, hourglass MLP).
    ...
    # Crown MLP — narrow at ends, wide in middle (Baroian & Notebomer 2025 best variant
    # at 180M scale). Default for 12L is 2 narrow / 8 wide / 2 narrow, which gives the
    # SAME total parameter count as the Frame variant (8 wide blocks + 4 narrow blocks).
    # This is the fair head-to-head comparison: identical parameter budget, opposite shape.
    # Override via env: HOURGLASS_MLP_MULTS="1,1,2,2,2,2,2,2,2,2,1,1"
    hourglass_mlp_mults: str = os.environ.get("HOURGLASS_MLP_MULTS", "1,1,2,2,2,2,2,2,2,2,1,1")
