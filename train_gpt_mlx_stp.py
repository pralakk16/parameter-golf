...
class Hyperparameters:
    ...
    # Constrains hidden states to change smoothly through the sequence.
    # Based on the geodesic hypothesis: token representations trace smooth
    # curves on a semantic manifold. Zero extra parameters.
    stp_beta: float = float(os.environ.get("STP_BETA", 0.1))  # weight of STP loss (0 = disabled)
...
class GPT(nn.Module):
    ...
    # - MODIFIED: STP (Semantic Tube Prediction) auxiliary loss
    def __init__(self, vocab_size: int, num_layers: int, dim: int, num_heads: int, num_kv_heads: int, mlp_mult: int,
                 logit_chunk_tokens: int, logit_softcap: float, rope_base: float, tied_embed_init_std: float,
                 qk_gain_init: float, stp_beta: float = 0.0, stp_layer: str = "last"):
        super().__init__()
